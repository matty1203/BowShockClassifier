...
Date Submitted: 10th August, 2021}
...
From Dataset 3, we have sampled 300 NE points, 100 Bow shock data points and 100 Magnetopause data points for the train data. Since there are 218 predictors in this dataset it is important to have more than 300 datapoints in the training dataset because  if predictor count is nearly as big as total data points then the linear regression is too flexible and it will overfits the data. Table 3 shown below is the confusion matrix of Logistic Regression Model with Dataset 3.
...
From Table 4, we can clearly see that the accuracy is about `r round(conf_mat_log2$overall['Accuracy']*100,1)`% . Eventhough it gives better accuracy than the Model with Dataset 3 but still the sensitivity of the Model is similar to that of the Model with Dataset 3. `r round(conf_mat_log2$byClass['Sensitivity']*100,1)`% of the Bow Shock boundaries were classified correctly. When compared to model that was trained with Dataset 3, the `NE` points were classified more accurately in this model.The specificity of this model is `r round(conf_mat_log2$byClass['Specificity']*100,1)`% which means the `r round(conf_mat_log2$byClass['Specificity']*100,1)`% of the NE Points were classified correctly through this model.


__Summary of Dataset 3 : Without Lead Variables__ 

In this dataset there are only 107 predictors which means half of the variables were removed so, the train dataset contains 100 NE points, 100 BS Points and 100 MP points which was sampled from the Dataset 4. Table 5 shown below is the Confusion matrix of the model that is trained with Dataset 3(without lead variables). A dataset without Lead variables means all the columns which contains only the magnetic field information before 15 minutes of each datapoint.
...
From Table 5, we can clearly see that the accuracy is about `r round(conf_mat_log3$overall['Accuracy']*100,1)`% . It gives better accuracy and sensitivity than the Model trained with the Dataset 3. `r round(conf_mat_log3$byClass['Sensitivity']*100,1)`% of the Bow Shock boundaries were classified correctly. But when compared to Model that was trained with Dataset 3, non Bow shock Classes were classified more accurately in this model.The specificity of this model is `r round(conf_mat_log3$byClass['Specificity']*100,1)`% which means the `r round(conf_mat_log3$byClass['Specificity']*100,1)`% of the NE Points were classified correctly through this model.

## Logistic Regression Model 2: Magnetopause vs Other Events

Unlike the previous model which classifies Bow Shock events from all other Events, This model classify Magnetopause event from rest of the events by using different derived datasets. Since the datasets are highly imbalanced It is important to sample equal number of classes from each dataset to train different Logistic Regression model. Before splitting into training and Test Dataset, A new variable called `event_occured` was created and all Magnetopause events were stored as 1 and all the rest of the events as 0 in that newly created variable.

### Results

Logistic Regression models were trained with different derived datasets and later the results of each model was compared .Table 6 contains the information regarding the datasets used and count of Variables that are significant. A brief summary of models trained with different dataset was explained in this section.
...
From Dataset 3, I have sampled 300 NE points, 100 Bow shock points and 100 Magnetopause Data points as the train data.Since there are 218 predictors in this dataset it is crucial to have more than 300 dtapoints in the train dataset. If predictor count is nearly as big as total data points then the linear regression is too flexible and overfits the data.Table 7 shown below is the confusion matrix of this model.
...
\end{center}
From Table 8, we can clearly see that the accuracy is about `r round(conf_mat_log5$overall['Accuracy']*100,1)`% . When comparing with the model that was trained with Dataset 3, this model did a vary good job in classifying the Magneto pause events. `r round(conf_mat_log5$byClass['Sensitivity']*100,1)`% of the Magnetopause boundaries were classified correctly. The `NE` points were also classified more accurately in this model.The specificity of this model is `r round(conf_mat_log5$byClass['Specificity']*100,1)`% which means the `r round(conf_mat_log5$byClass['Specificity']*100,1)`% of the NE Points were classified correctly through this model.


__Summary of Dataset 3 : Without Lead Variables__ 

In this dataset there are only 107 predictors which means half of the variables were removed so, the train dataset contains 100 NE points, 100 BS Points and 100 MP points from the Dataset 4. Table 9 Shown below is the Confusion matrix of the model with this dataset.
...
In Table 10 , LR Model 1 represents the Logistic Regression Model that classifies Bow Shock Events from all other events and LR Model 2 represents the Logistic Regression Model that classifies Magneto Pause events from all other events. From the table we can conclude that the Models that were trained with Dataset 4 and Dataset 3 (without Lead Variables) performed better than the model that is trained with Dataset 3 .
\newpage

# Classification Model Development

This section explains about the model that was developed for the classification of different boundaries based on the Magnetometer data. In the Predictor Significance section, we found that Dataset 4(Average Dataset) and Dataset 3(without lead variables) performed better when compared to  Dataset 3. The results of classification models that were trained with different datasets was explained  in this section and based on results a final  model will be selected that can be used to address the problem statement. The model with high sensitivity values and  better accuracy will be considered as the best Model. Because sensitivity value of each class will tell us the percentage of each correctly classified category . Several methods can be used for classifying the boundaries by using these kind of datasets, but in this project we have used Random forest model and Lasso regression Model.
...
The accuracy of the Random Forest Model with Dataset 3 is about `r round(conf_mat.rfmodel1$overall['Accuracy']*100,1)`% . The sensitivity value for each class which is `r round(conf_mat.rfmodel1$byClass['Class: BS','Sensitivity']*100,3)`% , `r round(conf_mat.rfmodel1$byClass['Class: MP','Sensitivity']*100,3)`%  and `r round(conf_mat.rfmodel1$byClass['Class: NE','Sensitivity']*100,3)`% for the Bow Shock, Magnetopause and No Events occurred Class respectively. Among these values the sensitivity of the Bow Shock and Magnetopause class is very low.



__Summary of Dataset 4 : Average and Standard Deviation Data__ 

This dataset contains only 24 predictors so, 100 NE points, 100 BS Points and 100 MP points were sampled from the Dataset 4. The table shown below is the Confusion matrix of the model with this dataset.
...
From the matrix we can clearly see that the accuracy is about `r round(conf_mat.rfmodel2$overall['Accuracy']*100,3)`% .`r round(conf_mat.rfmodel2$byClass['Class: BS','Sensitivity']*100,3)`% of the Bow Shock boundaries and `r round(conf_mat.rfmodel2$byClass['Class: MP','Sensitivity']*100,3)` % of Magnetopause boundaries were classified correctly through this model. 


__Summary of Dataset 3 : Without Lead Variables__ 

In this dataset there are only 107 predictors which means half of the variables were removed so, 100 NE points, 100 BS Points and 100 MP points were sampled from the Dataset 3 as the training data. The table shown below is the Confusion matrix of the model with this dataset.
...
This model performed better in classifying Magnetopause and Bow shock events when compared to other random forest models that were trained with Dataset 3 and Dataset 4. The sensitivity of bow shock and magnetopause events are `r round(conf_mat.rfmodel3$byClass['Class: BS','Sensitivity']*100,3)` % and `r round(conf_mat.rfmodel3$byClass['Class: MP','Sensitivity']*100,3)` % respectively. But the overall accuracy is still very low which is only `r round(conf_mat.rfmodel3$overall['Accuracy']*100,3)`%.    
...
The accuracy of the Lasso Regression Model with Dataset 3 is about `r round(conf_mat.lsmodel1$overall['Accuracy']*100,1)`% . The sensitivity values are `r round(conf_mat.lsmodel1$byClass['Class: 1','Sensitivity']*100,3)`% , `r round(conf_mat.lsmodel1$byClass['Class: 2','Sensitivity']*100,3)`%  and `r round(conf_mat.lsmodel1$byClass['Class: 0','Sensitivity']*100,3)`% for the Bow Shock, Magnetopause and No Events classes respectively. Among these values the sensitivity of the Bow Shock and Magnetopause class is very low which means this model failed to classify majority of the events correctly.  The Classes 0,1 and 2 in the confusion matrix is actually representing the `NE`,`BS` and `MP` events. 
...
The accuracy of this Lasso Regression Model which was trained with Dataset 4 is about `r round(conf_mat.lsmodel2$overall['Accuracy']*100,1)`% . The sensitivity values are `r round(conf_mat.lsmodel2$byClass['Class: 1','Sensitivity']*100,3)`% , `r round(conf_mat.lsmodel2$byClass['Class: 2','Sensitivity']*100,3)`%  and `r round(conf_mat.lsmodel2$byClass['Class: 0','Sensitivity']*100,3)`% for the classes Bow Shock, Magnetopause and No Events occurred respectively. When compared to model trained with Dataset 3, this model misclassified more `NE` points.
...
This model failed to classify majority of Magnet pause events when compared to other Lasso Regression models mentioned in this section. The accuracy of this Lasso Regression Model which was trained with Dataset 3 without lead variables is about `r round(conf_mat.lsmodel3$overall['Accuracy']*100,1)`% .

\newpage
## Overall Results

DIfferent models were fitted by using the datasets that were derived from the raw datasets. All the models were trained by using the balanced datasets that were sampled from the derived dataset that was intended for the model. Rest of the data points in that dataset were used as the test Dataset to find the accuracy and Sensitivity of the model.
...
## Supporting code & Supporting Files Link

\bigskip
__GitHub Repository__:https://github.com/matty1203/BowShockClassifier

\bigskip
__Dataset OneDrive Link__: https://maynoothuniversity-my.sharepoint.com/:f:/r/personal/mathews_philipvenattu_2021_mumail_ie/Documents/Thesis%20(20250487)%20Dataset?csf=1&web=1&e=AV79IL 

\bigskip

__SUPPORTING FILES__:https://maynoothuniversity-my.sharepoint.com/:f:/g/personal/mathews_philipvenattu_2021_mumail_ie/EkS2w7AD6-lHhegHiKzbwa4BHCWZbaMdCmytNFSNchfeNQ?e=FOzROg
